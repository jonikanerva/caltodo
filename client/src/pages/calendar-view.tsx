          ...
          {viewMode === "week" ? (
              ...
              {weekDays.map((day) => {
                ...
                return (
                  <div key={day.toISOString()} className="border-l relative">
                    <div
                      className={`h-12 border-b p-2 text-center ${dayIsToday ? "bg-primary/10" : "bg-muted/20"}`}
                    >
                      <div className="text-xs text-muted-foreground">
                        {format(day, "EEE")}
                      </div>
                      <div
                        className={`text-sm font-medium ${dayIsToday ? "text-primary" : ""}`}
                      >
                        {format(day, "d")}
                      </div>
                    </div>
                    <div
                      className="relative"
                      style={{ height: `${displayHours.length * 64}px` }}
                    >
                      {displayHours.map((hour) => (
                        <div
                          key={hour}
            ...
            <div className="grid grid-cols-2 border rounded-md overflow-hidden">
              ...
              <div className="border-l relative">
                <div
                  className="relative"
                  style={{ height: `${displayHours.length * 80}px` }}
                >
                  {displayHours.map((hour) => (
                    <div
                      key={hour}
        ...
        <Card>
          <CardContent className="p-8 text-center">
            <p className="text-muted-foreground">
              No scheduled tasks to display. Create tasks and configure your calendar to
              see them here.
            </p>
          </CardContent>
        </Card>
